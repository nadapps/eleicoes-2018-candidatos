...
const MenuImage = ({ navigation }) => {
  if (!navigation.state.isDrawerOpen) {
    return (
      <MaterialCommunityIcons name="menu" size={30} color={colors.black} />
    );
  } else {
    return (
...
const MainTab = createDrawerNavigator(
  {
    ...
    Estados: {
      ...
      navigationOptions: {
        drawerLabel: 'Estados',
        drawerIcon: ({ tintColor }) => (
          <MaterialCommunityIcons name="map" size={20} color={tintColor} />
        )
      }
    }
    // Partidos: {
...
const CandidatoTab = createBottomTabNavigator(
  {
    ...
    Candidaturas: {
      ...
      navigationOptions: {
        ...
        tabBarIcon: ({ tintColor }) => (
          <MaterialIcons name="star" size={30} color={tintColor} />
        )
      }
    },
    Finanças: {
      ...
      navigationOptions: {
        ...
        tabBarIcon: ({ tintColor }) => (
          <FontAwesome5 name="coins" size={22} color={tintColor} />
        )
      }
    }
  },
...
CandidatoTab.navigationOptions = ({ navigation }) => {
  let headerTitle =
    navigation.state.routes[navigation.state.index].params.title;
  let headerRight =
    navigation.state.routes[navigation.state.index].params.headerRight;
  let headerColor =
    navigation.state.routes[navigation.state.index].params.headerColor;
