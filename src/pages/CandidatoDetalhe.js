  ...
  async componentDidMount() {
    ...
    for (let i = 0; i < result.sites.length; i++) {
      ...
      for (let j = 0; j < redes.length; j++) {
        if (
          result.sites[i].includes('facebook') &&
          redes[j].includes('facebook')
        )
          contain = true;
        if (
          result.sites[i].includes('instagram') &&
          redes[j].includes('instagram')
        )
          contain = true;
      ...
      >
        <Row>
          <Col>
            ...
            <NumeroUrna
              ...
              numero={this.state.candidato.numero + ''}
            />
          </Col>
        </Row>
        <Row>
        ...
        <Row>
          <Col>
            ...
            <ItemCandidato
              title="Nome Completo:"
              value={this.state.candidato.nomeCompleto}
            />
            <ItemCandidato
              title="Ocupação:"
              value={this.state.candidato.ocupacao}
            />
            <ItemCandidato
              title="Sexo:"
              value={this.state.candidato.descricaoSexo}
            />
            <ItemCandidato
              title="Nascimento:"
              value={this.state.candidato.dataDeNascimento
                .split('-')
                .reverse()
                .join('/')}
            />
            <ItemCandidato
              title="Coligação:"
              value={this.state.candidato.nomeColigacao}
            />
            <ItemCandidato
              title="Partidos:"
              value={this.state.candidato.composicaoColigacao}
            />
          </Col>
        </Row>
            ...
            <Row>
              <Col size={30}>
                <Image
                  ...
                  source={{ uri: this.state.candidato.vices[0].urlFoto }}
                />
              </Col>
              <Col size={70}>
                <Text style={styles.titleVice}>
                  {this.state.candidato.vices[0].nm_URNA}
                </Text>
                <Text style={styles.subtitle}>
                  {this.state.candidato.vices[0].sg_PARTIDO}
                </Text>
              </Col>
            </Row>
          </View>
