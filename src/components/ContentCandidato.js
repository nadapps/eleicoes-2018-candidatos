...
class ContentCandidato extends React.Component {
	...
	render() {
        ...
        if(this.props.candidato.partido.sigla) {
            bandeiraPartido = bandeirasPartidos[this.props.candidato.partido.sigla.toLowerCase().replace(/\s+/g, "")]
            backgroundColor1 = coresPartidos[this.props.candidato.partido.sigla.toLowerCase().replace(/\s+/g, "")]
        }

        return (
            <Content loading={this.props.loading} style={{backgroundColor:backgroundColor1}} statusColor={backgroundColor1}>
                <View style={{position:'absolute',left:0,top:10,flex:1,flexDirection:'row',zIndex:1000}}>
                    <Row>
                        <Col style={{alignItems:'flex-end',justifyContent:'flex-end'}} size={50}>
                            <ImageProgress
                                indicator={Circle} 
