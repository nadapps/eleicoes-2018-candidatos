...
export const cargos = async estado => {
  let url =
    'http://divulgacandcontas.tse.jus.br/divulga/rest/v1/eleicao/listar/municipios/2022802018/' +
    estado +
    '/cargos';
...
export const candidatos = async (estado, cargo) => {
  ...
  let url =
    'http://35.227.86.242/eleicoes/api.php?funcao=candidatos&id_cargo=' +
    cargo +
    '&local=' +
    estado;
  return await get(url);
...
export const candidatosApuracao = async (estado, cargo) => {
  if (estado == 'DF' && cargo == 7) cargo = 8;
  let url =
    'http://divulga.tse.jus.br/2018/divulgacao/oficial/297/dadosdivweb/' +
    estado +
    '/' +
    estado +
    '-c000' +
...
export const candidatoGasto = async (
  estado,
  cargo,
  partido,
  numero,
...
export const partidos = async () => {
  let url =
    'http://divulgacandcontas.tse.jus.br/divulga/rest/v1/eleicao/2022802018/BR/2/partidos';
  return await get(url);
};
